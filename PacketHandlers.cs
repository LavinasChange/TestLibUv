...
namespace Tela
{
  ...
  {
    private static PacketHandler[] Handlers { get; set; }

    static PacketHandlers()
    {
      Handlers = new PacketHandler[0x100];

      Register(0x80, 62 - 1, false, AccountLogin);
      Register(0xEF, 21 - 1, false, LoginServerSeed);
    }

    static void Register(int id, int length, bool ingame, OnPacketReceive onReceive)
    {
      Handlers[id] = new PacketHandler(id, length, ingame, onReceive);
    }
    ...
    {
      Console.WriteLine("Login Seed 0xEF");
    }

    public static PacketHandler GetHandler(int id)
    {
      return Handlers[id];
    }
