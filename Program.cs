    ...
    static void Main(string[] args)
    {
      ...
      Task.Run(async () =>
      {
        ...
        while (true)
        {
          ...
          IMemoryOwner<byte> owner = MemoryPool<byte>.Shared.Rent(length - 1);
          readResult.Buffer.FirstSpan.Slice(2, length - 1).CopyTo(owner.Memory.Span);

          uvThread.PostAsync((Tuple<ConnectionContext, IMemoryOwner<byte>> t) =>
          {
            // stuff
            var (conn, memOwner) = t;
            // Do stuff wtih memOwner.Memory.Span;
            memOwner.Dispose();
          }, Tuple.Create(connectionContext, owner));
